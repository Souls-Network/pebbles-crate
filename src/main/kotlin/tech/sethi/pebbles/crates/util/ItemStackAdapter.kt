...
class ItemStackTypeAdapter : JsonSerializer<ItemStack>, JsonDeserializer<ItemStack> {
    override fun serialize(
        ...
        jsonObject.addProperty("itemId", BuiltInRegistries.ITEM.getId(itemStack.item).toString())
        ...
        if (itemStack.componentsPatch.size() > 0) {
            val nbtString = DataComponentPatch.CODEC.encodeStart(PebblesCrate.nbtOps, itemStack.componentsPatch).result().orElse(null)
